  ...
  async run({ ctx }) {
    ...
    const games = {
      valorant: async() => {
        ...
        const winnerIndex = user.valorant_predictions[index].teams.findIndex(t => t.winner)
        await ctx.reply(
          'helper.bet_res',
          {
      ...
      lol: async() => {
        ...
        const index = user.lol_predictions.findIndex(p => p.match === ctx.args[2])
        let odd: number
        ...
        const winnerIndex = user.lol_predictions.map(p => p.teams.findIndex(t => t.winner)).join()
        await ctx.reply(
          'helper.bet_res',
          {
