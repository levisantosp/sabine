...
export default class LeaderboardCommand extends Command {
  ...
  async run(ctx: CommandContext) {
    if(ctx.callback.data.options.getString('type') === 'local') {
      ...
      .setEmoji('◀️')
      .setCustomId(`leaderboard;${ctx.callback.member?.id};${page - 1 < 1 ? 1 : page - 1};previous;local`)
      .setStyle('gray')
      const next = new ButtonBuilder()
      .setEmoji('▶')
      .setCustomId(`leaderboard;${ctx.callback.member?.id};${page + 1 > Math.ceil(array.length / 10) ? Math.ceil(array.length / 10) : page + 1};next;local`)
      .setStyle('gray')
